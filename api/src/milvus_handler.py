...
class MilvusHandler:
    ...
    def __enter__(self):
        connections.connect(alias=MILVUS_DB_ALIAS, uri=MILVUS_DB_URI, host=MILVUS_DB_PORT)
    ...
    def get_similar_embeddings(self, embeddings: np.ndarray):
        ...
        similar_embeddings = self.collection.search(embeddings, search_field, param=search_params, limit=search_limit, expr=search_expr, timeout=MILVUS_DB_CONNECTION_TIMEOUT)
